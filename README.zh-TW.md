...
- **Media**：AVRCP 媒體鍵與音量自動化循環測試。
- **HFP / SCO Stress**：手動與自動 A2DP 至 HFP (通話模式) 切換測試。
- **Battery**：支援背景執行的電量與訊號強度 (RSSI) 記錄，並自動**匯出為結構化 CSV**。
...
功能：
- **自動 CSV 記錄**：所有的電量與 RSSI 數據都會自動存入 `Downloads/BT_Android_Agent_Logs/BatteryLog_YYYYMMDD.csv`。
