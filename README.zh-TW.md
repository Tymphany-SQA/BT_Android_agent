...
- **HFP / SCO Stress**：手動與自動 A2DP 至 HFP (通話模式) 切換測試，並追蹤 SCO transition latency。
...
- **連線 KPI 統計**：
  ...
  - 即時計算 **平均值 (Avg)**、**最大/最小值 (Min/Max)**、**90th Percentile (P90)** 與 **95th Percentile (P95)**。
  - 統計連線成功率。
  - 若有麥克風權限，會用短 440Hz probe 量測從 reconnect 開始到 **A2DP connected 且聲學輸出被偵測到** 的 recovery time。
  - 測試結束後輸出完整 KPI 摘要。
- 支援不同測試音色
- 可複製 log
- 可清空 log
...
功能：
- 發送標準媒體鍵事件
- 追蹤可觀測狀態變化的 command response time，例如音量變化與 music active 狀態變化。
- `Next` / `Prev` 目前記錄 dispatch timing，因為若沒有更深的播放器 metadata 整合，無法可靠判定曲目是否真的切換。
- 直接調整系統媒體音量
- 支援以語音檔做左右聲道與交替播放驗證
- 支援快速重複媒體鍵測試
- 支援在自訂音量範圍內做循環調整
...
- **Automation Settings** card
  ...
  - `Stop Loop`
- log card
  - SCO transition latency 紀錄

功能：
- 可手動切換 SCO on/off
- 可在 A2DP 與 HFP 模式之間做自動輪替
- 量測從 SCO request 到 Android SCO connected/disconnected broadcast 的 profile transition latency。
...
- **Microphone Analysis** 卡片
  ...
  - 左聲道狀態與 magnitude 顯示
  - 右聲道狀態與 magnitude 顯示
  - Manual Stats，顯示目前 mode 的 L/R uptime、lost count 與平均 magnitude
  - Auto Diag PASS/FAIL 摘要，包含各 mode 的 uptime 與 lost count
- **Event Log** 卡片
  - 含時間戳的偵測事件紀錄
...
- Manual mode 會即時統計目前 mode，並在停止或切換 mode 時寫入 `MANUAL_*` summary。
- `Auto Diag` 會自動輪跑 left-only、right-only、normal、swapped-left-only、swapped-right-only 與 full swap 檢查。
- Auto Diag 會將各 mode 彙整為分開的 `Result` 與 `Stability`，例如 `Result=PASS, Stability=UNSTABLE`。
- Auto Diag 每個 mode 前 500 ms 會視為 warm-up，不列入統計，避免切換瞬間拉低分數。
- Auto Diag 將 `Result`（`PASS` / `FAIL_*`）與 `Stability`（`STABLE` / `UNSTABLE`）分開輸出，避免 `PASS_UNSTABLE` 這類容易誤解的結果。
- Auto Diag 會將結構化結果寫入 `Downloads/BT_Android_Agent_Logs/AcousticDiag_YYYYMMDD.csv`。
...
4. 同時顯示 **latency jitter** 與 **P95 latency**，讓平均值正常但長尾不穩的情況更容易被看見。
5. 這種多輪量測法有助於過濾掉系統瞬時的抖動 (Jitter)，提供更可靠的藍牙音訊路徑評估數據。
...
### 11. Test Summary 與 Session ID
App 啟動時會建立一組 session ID，並將高階 KPI 摘要寫入 `Downloads/BT_Android_Agent_Logs/TestSummary_YYYYMMDD.csv`。

Summary 來源包含：
- Stress Test 的連線 KPI 與 recovery 統計。
- Audio Latency 的平均值、jitter 與 P95。
- Acoustic Auto Diag / Manual 的 result 與 stability。
- HFP / SCO transition latency。
- Media Control command response timing。

可用時，相同 session ID 也會寫入結構化 KPI CSV，方便把同一次驗證流程產生的多份 log 對齊。
